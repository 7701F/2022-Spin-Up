	...
	for (int i = 0; i < sample_size; i++) {
		...
		if (obj.area < data[i].area) {
			if (i == 0) {
				...
				obj.apprx_distance =
				    ((data[i - 1].distance - data[i].distance) / (data[i].area - data[i - 1].area)) * (obj.area - data[i - 1].area) +
				    data[i].distance;
			}
			break;
		}
...
visiondetect::Object::Object(pros::vision_signature_s_t sig, float ratio, int samples = 5, int brightness = 55, int min_area = 50,
                             float ratio_range = 0.15) {
	this->signature = sig;
	this->ratio = ratio;
